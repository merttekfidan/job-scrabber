                    ...
                    <div className="relative z-10">
                        ...
                        <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
                            <div className="bg-black/20 p-4 rounded-xl border border-white/5 backdrop-blur-sm">
                                <div className="text-gray-500 text-base uppercase tracking-wider font-bold mb-1 flex items-center gap-1"><MapPin size={12} /> Location</div>
                                <div className="text-white font-medium truncate">{app.location || 'Remote'}</div>
                            </div>
                            <div className="bg-black/20 p-4 rounded-xl border border-white/5 backdrop-blur-sm">
                                <div className="text-gray-500 text-base uppercase tracking-wider font-bold mb-1 flex items-center gap-1"><Banknote size={12} /> Salary</div>
                                <div className="text-white font-medium truncate">{app.salary ? formatSalary(app.salary, app.location) : 'N/A'}</div>
                            </div>
                            <div className="bg-black/20 p-4 rounded-xl border border-white/5 backdrop-blur-sm">
                                <div className="text-gray-500 text-base uppercase tracking-wider font-bold mb-1 flex items-center gap-1"><Briefcase size={12} /> Work Mode</div>
                                <div className="text-white font-medium truncate">{app.work_mode || 'Full-time'}</div>
                            </div>
                            <div className="bg-black/20 p-4 rounded-xl border border-white/5 backdrop-blur-sm">
                                <div className="text-gray-500 text-base uppercase tracking-wider font-bold mb-1 flex items-center gap-1"><Calendar size={12} /> Applied On</div>
                            ...
                            <div className="bg-black/20 p-4 rounded-xl border border-white/5 backdrop-blur-sm col-span-2 lg:col-span-1">
                                <div className="text-gray-500 text-base uppercase tracking-wider font-bold mb-1 flex items-center gap-1"><Globe size={12} /> Industry/Size</div>
                                <div className="text-white font-medium truncate">{companyInfo?.industry || 'Unknown Sector'}</div>
                                <div className="text-gray-400 text-base mt-1 truncate">{companyInfo?.size || 'Unknown Size'} • {companyInfo?.fundingStage || 'Unknown Funding'}</div>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="lg:col-span-2 space-y-8">
                            ...
                            <div className="bg-gradient-to-r from-blue-900/20 to-purple-900/20 border border-blue-500/20 rounded-2xl p-6 flex items-center justify-between">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-blue-300 text-base">{hiringManager.title || 'Hiring Manager'}</p>
                                    </div>
                                </div>
                                {hiringManager.linkedinUrl && hiringManager.linkedinUrl !== 'If available' && (
                                    <a href={hiringManager.linkedinUrl} target="_blank" rel="noopener noreferrer" className="px-4 py-2 bg-blue-600/20 hover:bg-blue-600/40 text-blue-300 rounded-lg text-base font-medium transition-colors">
                                        LinkedIn
                                    </a>
                                )}
                            </div>
                        ...
                        <div className="bg-gray-800/20 border border-gray-700/50 rounded-2xl p-6">
                            <div className="flex items-center justify-between mb-6">
                                ...
                                {!isShared && (
                                    <button
                                        className="bg-blue-600/20 hover:bg-blue-600/40 text-blue-400 border border-blue-500/30 px-3 py-1.5 rounded-lg text-base font-medium transition-colors flex items-center gap-2"
                                        onClick={() => {
                                                ...
                                                <div className="flex flex-col gap-3">
                                                    <div className="flex items-center justify-between gap-2">
                                                        {!isShared ? (
                                                            <div className="flex items-center gap-2 w-full">
                                                                <input
                                                                    type="text"
                                                                    className="bg-transparent font-bold text-white border-b border-transparent hover:border-gray-600 focus:border-blue-500 focus:outline-none w-1/3 text-base px-1 py-0.5 transition-colors"
                                                                    value={stage.round || ''}
                                                                    onChange={(e) => {
                                                                ...
                                                                <input
                                                                    type="date"
                                                                    className="bg-gray-900 border border-gray-700 rounded px-2 py-1 text-base text-blue-400 focus:outline-none focus:border-blue-500 w-1/3"
                                                                    value={stage.date || ''}
                                                                    onChange={(e) => {
                                                                        const updated = [...parseJson(app.interview_stages)];
                                                            ...
                                                            <>
                                                                ...
                                                                <span className="text-base font-medium text-blue-400 bg-blue-500/10 px-2 py-0.5 rounded-full">{formatDate(stage.date)}</span>
                                                            </>
                                                        )}
                                                    </div>
                                                    ...
                                                    {!isShared ? (
                                                        <textarea
                                                            ...
                                                            className="w-full bg-black/20 border border-gray-700/50 rounded-lg p-3 text-base text-gray-300 min-h-[80px] focus:outline-none focus:border-blue-500 transition-colors"
                                                            value={stage.notes || ''}
                                                            onChange={(e) => {
                                                                const updated = [...parseJson(app.interview_stages)];
                                                        ...
                                                        stage.notes && (
                                                            <div className="text-base text-gray-300 bg-black/20 p-3 rounded-lg border border-white/5 whitespace-pre-wrap">
                                                                {stage.notes}
                                                            </div>
                                                        )
                                                    )}
                            ...
                            <div className="bg-gray-800/30 border border-gray-700/50 rounded-2xl p-6">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {prep.techStackToStudy.map((tech, i) => (
                                        <span key={i} className="px-3 py-1 bg-yellow-400/10 text-yellow-200 border border-yellow-400/20 rounded-lg text-base font-medium">
                                            {tech}
                                        </span>
                                    ))}
                                </div>
                        ...
                        <div className="bg-gray-800/30 border border-gray-700/50 rounded-2xl p-6">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {parseJson(app.required_skills).map((skill, i) => (
                                    <span key={i} className="px-3 py-1 bg-blue-500/10 text-blue-300 border border-blue-500/20 rounded-lg text-base font-medium">
                                        {skill}
                                    </span>
                                ))}
                                {parseJson(app.required_skills).length === 0 && <span className="text-gray-500 italic text-base">No specific skills listed.</span>}
                            </div>
                        </div>
                            ...
                            <div className="bg-red-500/10 border border-red-500/30 rounded-2xl p-6">
                                ...
                                <ul className="space-y-3">
                                    {parseJson(app.negative_signals).map((signal, idx) => (
                                        <li key={idx} className="flex gap-2 text-red-200/80 text-base">
