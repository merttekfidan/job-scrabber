...
export default function HiringFrameworks({ appId, existingData, onDataUpdate }) {
    ...
    return (
        <div className="space-y-3 mt-6">
            <h4 className="text-base font-bold text-gray-400 uppercase tracking-wider mb-3">Hiring Frameworks</h4>
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                {!data ? (
                                    <button
                                        ...
                                        disabled={isLoading}
                                        className="px-3 py-1.5 bg-purple-600 hover:bg-purple-500 text-white text-base font-medium rounded-lg transition-colors disabled:opacity-50 flex items-center gap-1.5"
                                    >
                        ...
                        {isExpanded && data && (
                            <div className="p-4 pt-0 border-t border-gray-700/50 animate-in fade-in slide-in-from-top-2 duration-200" onClick={e => e.stopPropagation()}>
                                ...
                                <button
                                    onClick={(e) => { e.stopPropagation(); generate(key); }}
                                    disabled={isLoading}
                                    className="mt-3 text-base text-purple-400 hover:text-purple-300 underline disabled:opacity-50"
                                >
...
function FrameworkRenderer({ type, data }) {
    switch (type) {
        ...
        default: return <pre className="text-base text-gray-400 overflow-auto">{JSON.stringify(data, null, 2)}</pre>;
    }
}
            ...
            {(data.stories || []).map((story, i) => (
                <details key={i} className="group bg-gray-900/40 rounded-xl border border-amber-500/10 overflow-hidden">
                    <summary className="flex items-start gap-3 p-3.5 cursor-pointer list-none select-none hover:bg-gray-900/60 transition-colors">
                        <span className="text-amber-400 font-bold text-base mt-0.5 w-5 text-center shrink-0">
                            <span className="block group-open:hidden">+</span>
                            <span className="hidden group-open:block">−</span>
                        </span>
                        <div className="min-w-0">
                            <div className="text-base text-amber-400 font-bold uppercase tracking-wider mb-1">→ {story.targetRequirement}</div>
                            {story.transitionPhrase && <div className="text-base text-gray-500 italic line-clamp-1">"{story.transitionPhrase}"</div>}
                        </div>
                    </summary>
                    <div className="px-4 pb-4 pl-11 space-y-2 text-base border-t border-gray-700/30">
        ...
        <div className="space-y-4 pt-3">
            {data.keyConnections?.length > 0 && (
                <div className="space-y-2">
                    <div className="text-base text-gray-400 font-bold uppercase">Key Connections</div>
                    {data.keyConnections.map((conn, i) => (
                        <div key={i} className="flex items-center gap-2 text-base bg-gray-900/30 p-2 rounded-lg">
            ...
            <details className="group">
                <summary className="text-base text-purple-400 cursor-pointer hover:text-purple-300 transition-colors font-medium">
                ...
                <div className="bg-green-500/5 p-4 rounded-xl border border-green-500/10 mt-2">
                    <p className="text-gray-200 italic leading-relaxed text-base">&quot;{data.script}&quot;</p>
                </div>
            </details>
            {data.deliveryTip && (
                <div className="text-base text-gray-500 italic">💡 {data.deliveryTip}</div>
            )}
        </div>
    );
}
    ...
    return (
        <div className="space-y-4 pt-3">
            {data.marketRange && (
                <div className="grid grid-cols-3 gap-3">
                    <div className="bg-gray-900/40 p-3 rounded-xl text-center border border-gray-700/50">
                        <div className="text-base text-gray-500 mb-1">Low</div>
                    ...
                    <div className="bg-gray-900/40 p-3 rounded-xl text-center border border-emerald-500/20">
                        <div className="text-base text-gray-500 mb-1">Target</div>
                        <div className="text-lg font-bold text-emerald-400">{data.marketRange.mid}</div>
                    </div>
                    <div className="bg-gray-900/40 p-3 rounded-xl text-center border border-gray-700/50">
                        <div className="text-base text-gray-500 mb-1">High</div>
            ...
            {data.leveragePoints?.length > 0 && (
                <div>
                    <div className="text-base text-gray-400 font-bold uppercase mb-2">Your Leverage</div>
                    <div className="flex flex-wrap gap-2">
                        {data.leveragePoints.map((p, i) => (
                            <span key={i} className="px-2 py-1 bg-emerald-500/10 text-emerald-400 rounded text-base">{p}</span>
                        ))}
                    </div>
                </div>
            )}
                    ...
                    <div className="space-y-2 mt-2">
                        {Object.entries(data.negotiationScript).map(([key, value]) => (
                            <div key={key} className="bg-gray-900/40 p-3 rounded-lg border border-gray-700/30">
                                <div className="text-base text-gray-500 font-bold uppercase mb-1">{key.replace(/([A-Z])/g, ' $1').trim()}</div>
    ...
    return (
        <div className="space-y-4 pt-3">
                ...
                return (
                    <div key={key} className={`bg-${color}-500/5 p-4 rounded-xl border border-${color}-500/10`}>
                        <div className="flex items-center gap-2 mb-2">
                            <span>{emoji}</span>
                            <span className={`text-base text-${color}-400 font-bold uppercase`}>{label}</span>
                        </div>
                        <div className="text-base text-white font-medium mb-2">{phase.theme}</div>
            ...
            {data.presentationTip && (
                <div className="text-base text-gray-500 italic">💡 {data.presentationTip}</div>
            )}
        </div>
    );
}
        ...
        <div className="space-y-3 pt-3">
            {(data.questions || []).map((q, i) => (
                <details key={i} className="group bg-gray-900/40 rounded-xl border border-gray-700/50 overflow-hidden">
                    <summary className="flex items-start gap-3 p-3.5 cursor-pointer list-none select-none hover:bg-gray-900/60 transition-colors">
                        <span className="text-blue-400 font-bold text-base mt-0.5 w-5 text-center shrink-0">
                            <span className="block group-open:hidden">+</span>
                            <span className="hidden group-open:block">−</span>
                        </span>
                        <div className="min-w-0">
                            <div className="text-base font-medium text-white mb-1">&quot;{q.question}&quot;</div>
                    ...
                    <div className="px-4 pb-4 pl-11 space-y-2 border-t border-gray-700/30 pt-3">
                        <div className="text-base text-gray-400"><span className="text-green-400 font-bold">✓ Strategy:</span> {q.strategy}</div>
                        <div className="text-base text-gray-400"><span className="text-red-400 font-bold">✗ Avoid:</span> {q.redFlag}</div>
                    </div>
                </details>
            ))}
        </div>
