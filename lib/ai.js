...
export async function callGroqAPI(prompt, temperature = 0.2, userId = null) {
    ...
    if (userId) {
        try {
            const result = await query('SELECT groq_api_key FROM user_profiles WHERE user_id = $1', [userId]);
            if (result.rows.length > 0 && result.rows[0].groq_api_key) {
                apiKey = result.rows[0].groq_api_key;
            }
        } catch (e) {
            logger.warn('Failed to fetch user-specific Groq key', { error: e.message });
        }
    }

    if (!apiKey) {
        throw new Error('Please add your Groq API key in Settings to use AI features.');
    }
