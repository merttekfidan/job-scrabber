...
// Primary: Groq. Secondary: Claude. Then Gemini, OpenRouter.
...
export async function callAIWithPool(prompt, temperature = 0.2, userId = null) {
    ...
    if (userId) {
        try {
            ...
            if (result.rows.length > 0) {
                ...
                // Normalize priorities: providers with 99 (unchanged) get DEFAULT_PRIORITY order (Groq first)
                for (let i = 0; i < DEFAULT_PRIORITY.length; i++) {
                    const p = DEFAULT_PRIORITY[i];
                    const cfg = providersConfig[p];
                    if (cfg && (cfg.priority ?? 99) === 99) {
                        providersConfig = {
                            ...providersConfig,
                            [p]: { ...cfg, priority: i + 1 },
                        };
    ...
    // Order: Groq (default), Claude, Gemini, OpenRouter — same as DEFAULT_PRIORITY
    if (candidates.length === 0) {
        const SYSTEM_KEY_ENV = {
            groq: 'SYSTEM_GROQ_KEY',
            claude: 'SYSTEM_CLAUDE_KEY',
            gemini: 'SYSTEM_GEMINI_KEY',
            openrouter: 'SYSTEM_OPENROUTER_KEY',
        };
        const systemCandidates = [];
        for (const p of DEFAULT_PRIORITY) {
            const key = process.env[SYSTEM_KEY_ENV[p]];
            if (key) systemCandidates.push({ providerKey: p, apiKey: key });
        }
    ...
    for (const { providerKey, apiKey } of candidates) {
        ...
        const providerName = PROVIDER_CONFIGS[providerKey]?.name || providerKey;
        logger.info('AI bağlantısı deneniyor', { provider: providerName, model: '…' });
        ...
        try {
            ...
            logger.info('AI bağlantısı kuruldu', { provider: providerName });
            return result;
        } catch (err) {
...
function buildCandidateList(providersConfig) {
    ...
    // Sort: 1) by explicit priority (lower = higher), 2) by DEFAULT_PRIORITY (Groq first, Claude second)
