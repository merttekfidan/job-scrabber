...
const DEFAULT_PRIORITY = ['groq', 'gemini', 'openrouter', 'claude'];
...
async function callProvider(providerKey, apiKey, prompt, temperature = 0.2) {
    ...
    if (config.type === 'anthropic') {
        return callAnthropic(config, apiKey, prompt, temperature);
    }
...
async function callAnthropic(config, apiKey, prompt, temperature) {
    for (const model of config.models) {
        const response = await fetch(config.endpoint, {
            method: 'POST',
            headers: {
