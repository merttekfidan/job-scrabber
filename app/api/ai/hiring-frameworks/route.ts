...
type AppRow = {
  ...
  personalized_analysis?: Record<string, unknown> | string;
};

const FRAMEWORK_MAP: Record<
  string,
  (app: AppRow, cv: string) => string
> = {
  star: (app, cv) =>
    STAR_STORY_PROMPT(cv, app.formatted_content || app.original_content || ''),
  whyCompany: (app, cv) =>
    WHY_COMPANY_PROMPT(
      app.company || '',
      app.formatted_content || app.original_content || '',
      cv
    ),
  salary: (app, cv) =>
    SALARY_NEGOTIATION_PROMPT(
...
export async function POST(req: Request) {
  try {
    ...
    const body = (await req.json()) as { applicationId?: number; framework?: string };
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            'Invalid request. Provide applicationId and framework (star|whyCompany|salary|plan3060|competency).',
        },
        { status: 400 }
      );
    }
    ...
    const application = appResult.rows[0] as AppRow;
    ...
    const cvContent =
      cvResult.rows.length > 0
        ? ((cvResult.rows[0] as { raw_text?: string }).raw_text ?? '').substring(0, 5000)
        : 'No CV provided.';
    ...
    if (!(currentAnalysis as Record<string, unknown>).hiringFrameworks) {
      (currentAnalysis as Record<string, unknown>).hiringFrameworks = {};
    }
    ((currentAnalysis as Record<string, unknown>).hiringFrameworks as Record<string, unknown>)[
      framework
    ] = {
      data: parsed,
    ...
    logger.error('Hiring framework failed', { error: error instanceof Error ? error.message : String(error) });
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to generate framework',
      },
      { status: 500 }
    );
  }
